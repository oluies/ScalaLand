...
/** Race replaces both `object CreatureType extends Enumeration` AND the separate `Elf`/`Dwarf`
  * traits from `scalaland_immutable3`. The per-race feature distribution is a method on the enum
  * case rather than a trait override - no mixin needed.
  */
enum Race:
  case Elf, Dwarf
