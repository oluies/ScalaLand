...
/** A single immutable case class - no `with Elf with Thief` stacking, no abstract `Creature` trait,
  * no Avatar.apply factory cascade.
  *
  * Compare against `scalaland_immutable3.Avatar.apply` which has 25+ lines of nested `creature
  * match { case ... profession match { ... } }` to dispatch to one of six concrete mixin
  * combinations. Here the `case class` constructor IS the factory.
  */
case class Avatar(
    name: String,
    race: Race,
