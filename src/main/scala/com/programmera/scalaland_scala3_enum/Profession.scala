...
/** Profession replaces `object ProfessionalType extends Enumeration` AND
  * the `Thief`/`Warrior`/`Wizard` behaviour traits.  Each combat /
  * skill method is a single exhaustive `match` on the enum - the
  * compiler verifies completeness, no `case _` escape hatch.
  */
enum Profession:
